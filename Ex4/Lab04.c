...
{	ui_16 newtcnt;
 ...
 // If the tcnt value is zero and no previous count get out!
	...
	if (fred->count == 1) { // If first time through
		fred->min = newtcnt;
		fred->max = newtcnt;
		fred->mincount = 1;
		fred->maxcount = 1;
		TimeTestCheckStop(token); 
		}
	else if( newtcnt < fred->min) { // New minimum
			fred->min = newtcnt;
			fred->mincount = 1;
			}
	else if( newtcnt == fred->min ) {
			fred->mincount++;} //another at minimum
	else if( newtcnt > fred->max) { // New maximum
			fred->max = newtcnt;
			fred->maxcount = 1;
			}
	else if( newtcnt == fred->max ) {
			fred->maxcount++; } // another at maximum
    else {}
	}
}
